...
public class LoggerMap extends MapActivity
{
   ...
   protected void onCreate(Bundle load)
   {
      ...
      //根据配置文件mSharedPreferences，设置使用谁的mapView。然后对mapView进行一些基础的设置
      updateMapProvider();
      //创建一个overlay
   ...
   private void createListeners()
   {
       ...
       */
      //计算速度处理函数，最后会调到updateSpeedColoring，然后会更新路径图像
      speedCalculator = new Runnable()
      {
         public void run()
         {
            ...
            try
            {
               ...
               {
                  //取出记录的平均速度和最大速度的一半之中较小者，赋值给mAverageSpeed
                  //不清楚这里为啥要用这两个做比较
               ...
               if (avgspeed < 2)
               {
                  //如果速度小于2km/h，将其等于10km/h，这里的计算相当于是从km/h转换到了m/s
                  //但是下面又直接在用avgspeed 了，这个地方的目的还是不清楚
                  avgspeed = 5.55d / 2;
               }
            }
            finally
            ...
            //在UI进程里面调用updateSpeedColoring更新路径
      ...
      //普通视图还是卫星视图的listener，单选控件,这里会更新设置文件，然后通过配置文件的修改回调进行处理
      ...
      //选择要不要交通图之类的listener，多选控件,这里会更新设置文件，然后通过配置文件的修改回调进行处理
       ...
       */
      //用的SharedPreference做的配置文件，这里类似于配置文件改变
   ...
   //设置是使用google的mapView还是使用OSM的mapView，整体是通过一个代理类来屏蔽内部的实现
    ...
    * @see SegmentOverlay
    */
   //清除目前已有的段，重新生成一遍,每一段都对应一个overlay
   ...
   //更新overlay，如果现在已经没差就算了，否则重新生成一下overlay。每一个段对应一个overlay
